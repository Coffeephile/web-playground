...
const createAssetFileIfNotExistPromise = Bluebird.coroutine(function *(baseDir, type, tmplFactory, playground) {
  const joinBase = File.join(baseDir);
  const extensions = extensionsForAsset(type)(playground);

  try {
    yield PromiseUtil.tryInOrder(R.compose(File.readFile, joinBase, R.concat(`${type}.`)), extensions);
  } catch (err) {
    const file = joinBase(`${type}.${extensions[0]}`);
    Log.info(`Did not find ${file}, creating one...`);
    const content = yield Bluebird.resolve(tmplFactory(extensions[0]));
    yield File.writeFile(file, content);
    logCreateFile(file);
  }
});
...
module.exports = {
  ...
  createAssetFileIfNotExistPromise,
};
